...
/* Passive buzzer: GPIO9/PWM0 (traffic-light board) */
#define BUZZER_GPIO         9
#define BUZZER_PWM_PORT     0
#define BUZZER_DUTY         50

/* Traffic-light board LEDs: active high (output 1 = on) */
#define LED_RED_GPIO        10
#define LED_GREEN_GPIO      11
#define LED_GREEN_ON_MS     1000   /* safe open : green on for 1 second  */
#define LED_RED_ON_MS       3000   /* mine hit  : red on for 3 seconds    */
...
/* LED pulse timers: 0 = off, otherwise tick at which the LED goes off */
static uint32_t g_greenOffMs = 0;
static uint32_t g_redOffMs = 0;

/* -------------------- indicator lights (non-blocking) -------------------- */
...
/* Turn `pin` on now and schedule it off after durMs. The red light has
 * priority: firing red cancels green; green is ignored while red is on. */
static void LedPulse(uint8_t pin, uint32_t durMs, uint32_t now)
{
    ...
    if (pin == LED_RED_GPIO) {
        ...
        if (g_redOffMs != 0) {
            return;   /* red alarm in progress, keep it showing */
        }
...
/* Advance the LED timers; call once per main-loop iteration. */
...
/* mine hit: three rapid, urgent beeps (~0.5s, red light stays 3s) */
static const Beep g_sndBoom[]  = { { 2500, 90 }, { 0, 70 },
                                   { 2500, 90 }, { 0, 70 },
                                   { 2500, 130 } };
...
static void HandleKeys(uint32_t now)
{
    ...
    if (evAct & EV_SHORT) {
        Cell *c = &g_board[g_curY][g_curX];
        uint8_t willOpen = (!c->opened && !c->flagged);  /* a real open? */
        DoOpen();
        if (g_over == 1) {
            /* mine hit: red light for 3s + three rapid beeps */
            LedPulse(LED_RED_GPIO, LED_RED_ON_MS, now);
            SoundPlay(g_sndBoom, sizeof(g_sndBoom) / sizeof(g_sndBoom[0]));
        } else if (willOpen) {
            /* safe cell opened: green light for 1s */
            LedPulse(LED_GREEN_GPIO, LED_GREEN_ON_MS, now);
            if (g_over == 2) {
...
/* GPIO10/GPIO11: red/green traffic-light LEDs, active high */
...
static void ShowWelcome(void)
{
    ...
    printf("[Minesweeper] Lights: safe open -> green(GPIO11) 1s; mine hit -> red(GPIO10) 3s + 3 beeps.\r\n");
}
