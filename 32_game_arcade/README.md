...
> 本项目把**扫雷、贪吃蛇、弹球消方块**三款游戏整合到同一个固件中。上电（或按 reset 键）后先进入游戏选择菜单，选定后进入对应游戏；任意游戏中长按 USER 键可返回菜单。
...
```
minesweeper.c ─┐
snake.c       ─┼──► arcade.h ──► arcade.c（唯一接触 GPIO/ADC/PWM/I2C 的文件）
breakout.c    ─┘
```

- **arcade.c** 是唯一直接操作硬件（GPIO/ADC/PWM/I2C）的文件，对外只暴露 `KeysScan`、`SoundPlay`、`LedPulse`、`FillRect` 等语义化接口。
...
上电/reset 后显示游戏列表，`R1` 循环切换，`R2` 或 `USER` 短按确认进入。
...
所有游戏中 **USER 长按** 返回菜单；游戏结束后短按任意键返回菜单。
